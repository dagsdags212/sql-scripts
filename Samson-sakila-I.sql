...
SELECT first_name, last_name, email, store_id FROM customer AS email_address WHERE store_id = 2;
...
SELECT email, LENGTH(email) FROM customer ORDER BY LENGTH(email) DESC, email ASC;
SELECT email, LENGTH(email) FROM customer ORDER BY LENGTH(email) ASC LIMIT 100;